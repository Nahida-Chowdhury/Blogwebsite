  ...
  return (
    <div>
      <div className=' flex items-center justify-between px-6 md:px-[200px] py-4 bg-black text-white ' >
        <h1 className=' text-lg md:text-xl font-extrabold '> <Link to = "/">VibeWrite</Link>
        ...
        </h1>
        {
          path === "/" && <div onChange={(e) => setPrompt(e.target.value)} className= ' flex justify-center items-center space-x-0'>
            ...
            <p  onClick = {() => navigate(prompt ? "?search" + prompt : navigate("/") ) }  className=' cursor-pointer p-1 bg-white text-black rounded-l-xl'>
            <BsSearch/>
            </p> 
          </div>          
        }
